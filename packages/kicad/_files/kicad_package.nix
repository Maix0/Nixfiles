{
  ...
  majorVersion ? "10",
  versionsImportFile ? ./version10.nix,
}:
...
let
  baseName =
    if testing then
      ...
      "kicad-unstable";
  versionsImport = import versionsImportFile;
...
stdenv.mkDerivation rec {
  ...
  makeWrapperArgs =
    ...
    [
      ...
      "--set-default KICAD${majorVersion}_FOOTPRINT_DIR ${footprints}/share/kicad/footprints"
      "--set-default KICAD${majorVersion}_SYMBOL_DIR ${symbols}/share/kicad/symbols"
      "--set-default KICAD${majorVersion}_TEMPLATE_DIR ${template_dir}"
    ]
    ++ optionals (addons != [ ]) (
      let
      ...
      in
      [ "--set-default NIX_KICAD${majorVersion}_STOCK_DATA_PATH ${stockDataPath}" ]
    )
    ++ optionals with3d [
      "--set-default KICAD${majorVersion}_3DMODEL_DIR ${packages3d}/share/kicad/3dmodels"
    ]
